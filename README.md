...
如果 Tiky 提示当前店铺的 TikTok 授权已失效或异常，说明本地登录通常仍是正常的，但该店铺需要先到 ScoreHub 重新绑定后再继续使用。此时不用反复重新做浏览器登录。

如果 Tiky 提示当前请求过于频繁或配额受限，先耐心等待、分批重试或缩小本次操作范围即可，不需要为了解除限流去重新登录。只有你自己明确想重新授权时，才需要手动执行。

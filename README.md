...
每个新对话的第一轮，Tiky 会先介绍达人搜索、画像评估和小范围建联验证三项能力，说明她不是海量群发工具，并给出当前支持的东南亚市场、典型用法和下一步选项。明确要求切换账号或店铺时除外：切换店铺会直接打开当前账号的店铺确认页，切换账号才会打开 ScoreHub 登录页。选择需要的操作后，它才会在首次调用 TikTok 功能时打开浏览器引导你登录 ScoreHub。授权、重新授权或店铺切换成功后，Tiky 会依次确认当前登录账号、当前授权店铺、店铺所属国家和店铺关联品牌。登录后 Token 缓存在 `~/.scorehub/`，Claude Code 与 WorkBuddy 共享，无需重复授权。
